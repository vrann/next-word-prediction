...
getNextWords = function(sentence, num = 1)
{
    ...
    while (nrow(nextwordsIds) < num && length(tokens) >= 1) {
        idsVector = data.table(V1 = getPknHigher(tokens))
        #print(idsVector)
        if (nrow(nextwordsIds) == 0) {
            nextwordsIds = idsVector
        } else {
            nextwordsIds = rbind(nextwordsIds, idsVector[!(idsVector$V1 %in% nextwordsIds$V1)])
        }
    ...
    nextwordsIds = nextwordsIds[, list(V1, rid = seq_along(V1))]

    setnames(nextwordsIds, c('id', 'rid'))
    result = ngrams1[ngrams1$id %in% nextwordsIds$id]
    setkey(nextwordsIds, id)
    setkey(result, id)
    result = result[J(nextwordsIds)]
    result = result[order(rid)]$V1
    result
}
...
getPknHigher = function(tokens)
{
    ...
    if (order == 3) {
        ...
        result = result[order(pkn, decreasing=T), b]
    } else if (order == 2) {
        ...
        result = result[order(pkn, decreasing=T), a]
    } else if (order == 1) {
        ...
        result = result[order(pkn, decreasing=T), y]
    }
    
    result
}

getPknLower = function(tokens)
{
    order = length(tokens)
...
prepare = function(string)
{
    ...
    tkn = data.table(tkn[((length(tkn)-min(3, length(tkn))) + 1):length(tkn)])
